...
outdir=/data/tzc/model/CSA/lm-critic/CSA12
mkdir -p ${outdir}
echo "start training"
nohup python3.8 -u src/run_seq2seq.py \
    --model_name_or_path /data/tzc/model/benchmark/lm-critic \
    --task summarization --text_column bad_detoked --summary_column good_detoked \
    --do_train --num_train_epochs 39 \
    --train_file /data/tzc/data/finetune/gec/forget/lm-critic/CSA12/hard.json \
    --preprocessing_num_workers 20 --overwrite_output_dir \
    --output_dir $outdir --predict_with_generate --fp16 \
    --per_device_train_batch_size 128 --gradient_accumulation_steps 4 \
    --max_source_length 64 --max_target_length 64 \
    --logging_first_step --logging_steps 20 --save_steps 2000 > $outdir/log.txt 2>&1 &
